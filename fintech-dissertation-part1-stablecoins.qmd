...
Stablecoin domestic transaction costs are particularly prohibitive. Ethereum network fees alone can range significantly under normal conditions, with dramatic increases during high congestion periods. During the May 2022 NFT boom, Ethereum fees exceeded $200 [@bitinfocharts2025ethfees]. Complete stablecoin transaction chains require users to pay exchange trading fees (0.1-0.5%), network gas fees (<0.1% on Layer-2 networks^[Secondary frameworks built on top of main blockchains to improve transaction speed and reduce costs while maintaining security through the base layer] to 2-10% on Ethereum mainnet), and ZAR/USDT conversion spreads (1-3%), resulting in total costs of 2-7% for efficient routes or 4-15% via Ethereum for transactions under R1,000 [@luno2025fee ; @valr2025trading ; @binance2025; @ethereum2025; @transak2025]. This makes stablecoins economically unfeasible for the majority of South African retail transactions.^[South Africans use cash most often as a payment method irrespective of what they buy, as this is a commonly accepted payment method. The average value per payment across all payment methods is R529 [@sarb2023national], implying fees of between R10 and R80 per transaction.]

Brazil's mandated approach (detailed in Section 1.1) demonstrates how policy intervention can address these cost barriers. By mandating zero fees for person-to-person transfers and person-to-merchant transactions under R30-equivalent, Pix achieved the rapid adoption documented earlier [@duarte2022central; @bcb2024pix].
...
ggplot(remittance_data, aes(x = reorder(Country, Q3_2024_Cost_Percent), y = Q3_2024_Cost_Percent)) +
  ...
  labs(
    ...
    caption = paste("Source: Author's calculations from World Bank RPW Database Q3 2024",
                   ...
                   paste0("Sample: ", sum(remittance_data$Number_of_Services, na.rm=TRUE), " services across ", 
                          sum(remittance_data$Number_of_Corridors, na.rm=TRUE), " corridors"),
                   sep = "\n")
  ) +
  theme_minimal() +
  theme(
...
flextable(settlement_data) %>%
  ...
  add_footer_lines("Sources: PayShap/RTGS data from BankservAfrica (2024) and SARB (2024); Pix data from BCB (2024) and Duarte et al. (2022); Ethereum finality from Ethereum Foundation (2024); Bitcoin confirmation requirements from Nakamoto (2008) and Croman et al. (2016).") %>%
...
Network capacity represents another constraint differentiating traditional payment systems from blockchain-based systems. Major blockchain networks have limited throughput, with Bitcoin processing 3-7 TPS [@croman2016scaling] and Ethereum handling 12-15 TPS [@ethereum2024proof]. In contrast, traditional payment networks handle billions of transactions annually through centralized infrastructure [@visa2024annualreport]. Blockchain-based stablecoin systems face documented scalability constraints limiting transaction throughput [@croman2016scaling]. Layer-2 solutions^[Secondary frameworks built on top of main blockchains to improve transaction speed and reduce costs while maintaining security through the base layer] attempt to address these limitations, with Polygon achieving 34-38 transactions per second and Arbitrum processing over 20 transactions per second while maintaining security through fraud-proof mechanisms^[Security systems where transactions are assumed valid unless proven fraudulent within a challenge period] [@polygon2025chain]. However, even these enhanced throughputs remain orders of magnitude below traditional payment system capabilities.
...
The effectiveness of digital payment systems depends on solving the two-sided market problem. South Africa's merchant acceptance infrastructure reveals stark inequalities. The country has approximately 450,000 POS terminals serving a population of 60 million [@bis2025cpmisa], translating to roughly 7.7 terminals per 1,000 people – significantly below the OECD average of 33 terminals per 1,000 people^[Calculated from BIS data [@bis2025cpmistats]] . More critically, this infrastructure is heavily concentrated in urban areas and formal retail environments [@finmark2024finscope].

Research by Mastercard and Genesis Analytics reveals that approximately 90% of South Africa's 1.5 million informal enterprises operate as cash-only businesses, with only 10% accepting card or digital payments [@mastercard2017informal]. Despite rural and township residents using cards for 60% of their transactions at formal retailers, only 4% of transactions at informal retailers are card-based due to limited payment acceptance infrastructure [@mastercard2017informal]. The FinScope MSME South Africa 2024 Survey confirms this pattern, noting that "despite a marked increase in the use of digital financial services, the MSME ecosystem remains largely cash-driven," with 72% of micro-enterprises operating informally [@finmark2024msme].
...
inclusion_data <- data.frame(
  ...
  South_Africa = c("84.4", "73.2", "72.0", "94", "17", "7.7"),
...
Limited network effects compound these challenges. With cryptocurrency ownership at 10-13% of South African adults [@chainalysis2024global], merchants see limited economic incentive for adoption, contrasting with card acceptance where 73% consumer ownership creates clear demand [@worldbank2025global].
...
The empirical cost analysis presented earlier demonstrates that stablecoins could offer meaningful advantages for cross-border payments, though these benefits are constrained by practical implementation challenges. With traditional remittance costs from South Africa averaging 7.1% compared to the global average of 6.4%, the potential for stablecoin costs of 1-6% depending on transaction size represents a tangible improvement—particularly given that South Africa's costs exceed the UN Sustainable Development Goal target of 3% by more than double [@worldbank2024remittance].

Stablecoins achieve these potential cost reductions through direct peer-to-peer transfers that bypass traditional correspondent banking networks, eliminating the 2-3 intermediary banks typical in conventional systems [@auer2021multi]. Settlement times compress from 2-5 days to minutes depending on the blockchain used, with Bitcoin requiring approximately 60 minutes for secure confirmation and Ethereum achieving economic finality in 12-15 minutes [@ethereum2024proof; @nakamoto2008bitcoin]. 

However, realizing these theoretical advantages requires navigating significant practical challenges. As documented in the transaction cost analysis, complete stablecoin transaction chains involve multiple cost components: on-ramp costs of 0.5-2% in trading fees plus 1-3% spreads on ZAR pairs [@luno2025fee; @valr2025trading], network fees ranging from minimal amounts on Layer-2 solutions like Polygon to potentially exceeding $50 on Ethereum during congestion [@olaoluwa2023networkfees; @pierce2025polygonfees], and off-ramp costs that vary significantly by jurisdiction. The analysis showed total costs of 2-7% for efficient routes, which while potentially lower than the 7.1% average for traditional South African remittances, still exceed best-practice traditional corridors and far exceed the SDG target.
...
**Prioritize domestic payment infrastructure development.** The documented success of instant payment systems, particularly Brazil's Pix (Section 1.1), supports continued focus on traditional payment system enhancement as the primary vehicle for achieving domestic payment efficiency and financial inclusion objectives. This approach offers predictable benefits with implementation costs that are manageable for national systems while building upon existing institutional capabilities and relationships. The South African Reserve Bank should accelerate PayShap rollout with specific targets: achieving significant adult adoption within 24 months, onboarding informal merchants progressively, and maintaining low transaction costs for small-value payments.
